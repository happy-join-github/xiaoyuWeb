      ...
      <el-form ref="formRef" class="form fade-up" :model="ruleForm" :rules="rules" label-position="top">
        <el-form-item label="选择头像">
          <div class="avatar-picker">
            <div
              v-for="(src, i) in defaultAvatars"
              :key="i"
              class="avatar-option"
              :class="{ active: ruleForm.avatar === src }"
              @click="onPickDefault(src)"
            >
              <img :src="src" :alt="`默认头像${i + 1}`" />
            </div>
            <label class="avatar-option avatar-upload" :class="{ active: ruleForm.avatar && !defaultAvatars.includes(ruleForm.avatar) }">
              <input type="file" accept="image/*" hidden @change="onAvatarUpload" />
              <img v-if="ruleForm.avatar && !defaultAvatars.includes(ruleForm.avatar)" :src="ruleForm.avatar" alt="上传头像" />
        ...
        <el-form-item label="手机号" prop="phone">
          <el-input v-model="ruleForm.phone" type="tel" placeholder="11 位手机号" :maxlength="11"
            autocomplete="tel" clearable>
            <template #prefix>
              <span class="input-ic">📱</span>
            </template>
          </el-input>
